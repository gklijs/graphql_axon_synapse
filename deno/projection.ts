...
export const activeCards = async () => {
    return giftCards.find({cardId: {$ne: null}, remainingValue: {$ne: 0}, canceled: false}).toArray();
}
...
const redeemGiftCard = async (req: Request) => {
    ...
    await giftCards.updateOne(
        ...
        {$inc: {remainingValue: -payload['amount'], sequenceNumber: 1}, $set: {lastUpdated: date}},
    );
    cardIdEmitter.emit(payload['id']);
    return okResponse();
}
...
const cancelGiftCard = async (req: Request) => {
    ...
    await giftCards.updateOne(
        {cardId: payload['id'], sequenceNumber: sequenceNumber - 1},
        {$inc: {sequenceNumber: 1}, $set: {cancelled: true, lastUpdated: date}},
    );
    cardIdEmitter.emit(payload['id']);
    return okResponse();
}
...
export const initProjection = async () => {
    giftCards.createIndexes({indexes: [{"key": {"cardId": 1}, "name": "giftcard_cardid", "unique": true}]})
    const init = db.collection<InitSchema>("init");
    const currentInit = await init.find({activeEventHandler: {$ne: null}}).next();
    ...
    if (currentInit === undefined) {
        ...
        let reply = await fetch("http://localhost:8081/v1/contexts/default/handlers/events", {
            method: "POST",
